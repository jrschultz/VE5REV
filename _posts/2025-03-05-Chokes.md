...
Later research necessitated editing this post. Please see the update below.
...
In a recent hybrid balun project I discovered that using bifiliar windings as shown in this article has a terrible side effect. It causes a major amount of inductive reactance from 20M and up. The attenuation information is accurate for common mode current. **However, I now recommend using coax instead of the bifiliar windings.** Coax maintains 50 ohms throughout the HF spectrum, essentially making the choke "invisible" to the antenna system, other than supressing common mode interference.
...
As per the update above, **DO NOT USE bifiliar solid core wire** - use coax or teflon wire.
